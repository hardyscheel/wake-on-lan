...
DATA_FILE = "./docs/prototypes/wake-on-lan6/config/mac_addresses.json"
...
def wake_entry():
    ...
    if interface_table.selection():
        ip = interface_table.item(interface_table.selection())["values"][1]  # IP of the selected interface

    try:
        if mac and ip:
            send_magic_packet(mac, interface=ip)  # Specify interface IP
